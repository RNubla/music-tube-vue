...
export default {
  ...
  created() {
    window.addEventListener('beforeunload', this.endChildProcess())
  },
  beforeDestroy() {
    window.removeEventListener('beforeunload', this.endChildProcess)
  },
  methods: {
    play(event) {
    ...
    async endChildProcess(event) {
      event.preventDefault()
      // alert('you sure u wanna close')
      const command = new Command('youtube-dl')
